...
// flutter/services.dart: 텍스트 입력 필터링(예: 숫자만 입력) 같은 시스템 레벨의 기능을 사용하기 위해 필요합니다.
import 'package:flutter/services.dart';
// intl/intl.dart: 날짜 형식을 'yyyy.MM.dd'처럼 쉽게 변환하기 위해(DateFormat) 사용합니다.
import 'package:intl/intl.dart';
// table_calendar/table_calendar.dart: 화면에 달력을 그리기 위한 외부 라이브러리입니다.
import 'package:table_calendar/table_calendar.dart';
// ../models/ingredient.dart: 재료 데이터의 구조(Class)를 정의한 파일을 가져옵니다.
import '../models/ingredient.dart';
// ../widgets/ingredient_item.dart: 재료 목록의 각 항목을 그리는 UI 위젯을 가져옵니다.
import '../widgets/ingredient_item.dart';
// ../services/refrigerator_service.dart: 재료 데이터를 실제로 추가/수정/삭제/조회하는 로직이 담긴 파일을 가져옵니다.
...
class _RefrigeratorScreenState extends State<RefrigeratorScreen> {
  ...
  // [initState]
  // 이 위젯(화면)이 처음 생성될 때 딱 한 번 호출되는 함수입니다.
  @override
  void initState() {
    super.initState();
  ...
  void _refreshList() {
    // setState() : 괄호 안의 코드가 실행된 후 플러터에게 "화면을 다시 그려라"라고 알립니다.
    setState(() {
      // (A) 서비스에서 '모든' 재료 데이터를 가져옵니다.
    ...
    showDialog(
      ...
      builder: (context) { // 'context'는 다이얼로그가 그려질 위치(화면) 정보입니다.
        ...
        Widget buildDateTextField(TextEditingController controller, String hint, int maxLength) {
          return TextField(
            controller: controller, // 이 컨트롤러와 텍스트 필드를 연결
            maxLength: maxLength, // 최대 입력 글자 수 (예: 'YYYY'는 4)
            keyboardType: TextInputType.number, // 숫자 키보드만 표시
            inputFormatters: [FilteringTextInputFormatter.digitsOnly], // 숫자만 입력되도록 필터링
            textAlign: TextAlign.center, // 텍스트 가운데 정렬
            decoration: InputDecoration(
              hintText: hint, // 입력 예시 (예: "YYYY")
              counterText: '', // '4/4' 같은 글자 수 표시 숨김
              filled: true, // 배경색 채우기
              fillColor: Colors.blue[50], // 연한 파란색 배경
            ...
            TextButton(
              onPressed: () {
                ...
                try {
                  ...
                  DateTime date = DateTime(y, m, d); // 2025, 2, 30 -> 2025, 3, 1 (DateTime이 자동 보정)
                  ...
                  if (date.isBefore(todayOnly)) { // 선택한 날짜가 오늘 이전이면
                    _showErrorSnackBar("유통기한이 오늘보다 빠를 수 없습니다."); return;
                  }
                ...
                _refreshList(); // 12. 서비스 호출 후 화면 갱신 (가장 중요)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // 스크롤이 가능하도록 만듭니다. (달력 + 목록)
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(16.0), // 화면 전체의 바깥쪽 여백
        child: Column( // 자식 위젯들을 수직(세로)으로 배치
          children: [
            // [달력 위젯]
            Container( // 달력을 감싸는 컨테이너
              decoration: BoxDecoration(
                color: Colors.white, // 배경색 흰색
                borderRadius: BorderRadius.circular(16), // 모서리 둥글게
              ),
              child: TableCalendar(
                locale: 'en_US', // 달력 언어 (이 코드에서는 'en_US'지만, 'ko_KR'로 변경 가능)
                firstDay: DateTime.utc(2020, 1, 1), // 달력이 보여줄 수 있는 최소 날짜
                lastDay: DateTime.utc(2030, 12, 31), // 달력이 보여줄 수 있는 최대 날짜
                focusedDay: _focusedDay, // 현재 포커스된 날짜 (상태 변수)
                calendarFormat: _calendarFormat, // 달력 형식 (상태 변수)
                ...
                eventLoader: _service.getEventsForDay, //  14. 서비스 함수 호출
            ...
            const Divider(height: 10), // 구분선

            // [재료 목록 (ListView)]
            // filteredIngredients(상태 변수)의 내용에 따라 목록을 동적으로 생성합니다.
            ListView.builder(
              shrinkWrap: true, // true: SingleChildScrollView 안에서 스크롤 영역이 중첩되지 않도록
              // ListView의 높이를 자식 아이템 높이의 합만큼만 차지하도록 설정
              physics: const NeverScrollableScrollPhysics(), // true: ListView 자체의 스크롤을 막습니다.
              // (가장 바깥쪽 SingleChildScrollView가 스크롤을 담당)
              itemCount: filteredIngredients.length, // 목록 아이템의 총 개수
              ...
              itemBuilder: (context, index) {
                ...
                return Dismissible(
                  ...
                  // background: 스와이프할 때 '뒤'에 보이는 배경 (빨간색)
                  background: Container(color: Colors.red),
                  secondaryBackground: Container(color: Colors.red), // (오른쪽->왼쪽 스와이프 시 배경, 여기선 동일)
